...
# MiniMax base URL — set this in Render's Environment to your real endpoint.
# Common values: https://api.minimax.io/v1  or  https://api.minimaxi.chat/v1
...
2. Menciona SOLO las plataformas que tuvieron coincidencias.
3. Para cada coincidencia usa el enlace DIRECTO a la publicación (source_url).
4. Si la coincidencia trae un teléfono (campo "phone"), inclúyelo visible.
5. Si no hay coincidencias, dilo en 2-3 líneas y sugiere dónde más buscar.
6. NO uses formato markdown pesado — solo texto plano con líneas cortas.
7. NO des falsas esperanzas. Si aparece "missing" en la fuente, repite eso.
8. Incluye siempre una línea final recordando verificar en la fuente original.
...
async def format_results(query, results, language_hint):
...
def _deterministic_template(query, results):
            ...
            for r in with_matches:
                ...
                for m in r.matches[:5]:
                    ...
                    if seen:
                        lines.append(f"    {seen}")
                    if phone:
                        lines.append(f"    ☎ {'Teléfono' if es else 'Phone'}: {phone}")
                    lines.append(f"    🔗 {link}")
                lines.append("")
            if others > 0:
                lines.append(
                    f"También busqué en otras {others} plataformas sin coincidencias."
                    if es else
                    f"Also searched {others} more platforms with no matches."
                )
        else:
            lines.append(
                f"Entiendo lo difícil que es este momento. Busqué «{query}» en las "
                f"{len(results)} plataformas y por ahora no hay coincidencias. "
                "Los registros se actualizan a cada hora."
                if es else
                f"I searched «{query}» across {len(results)} platforms — no matches yet. "
                "Records update hourly."
            )
